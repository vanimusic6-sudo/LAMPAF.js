(function () {
    ...
    function initSettings() {
        try {
            ...
            [
                ...
                { name: 'ai_api_model', type: 'input', title: 'AI модель', description: 'Например: mixtral-8x7b-32768, llama-3.1-8b-instant, llama3.2, mistral', default: 'mixtral-8x7b-32768' }
            ].forEach(function(p) {
    ...
    function aiApiSettings() {
        return {
            ...
            model: String(getSetting('ai_api_model', 'mixtral-8x7b-32768') || '').trim()
        };
    }
    ...
    function llamaChat(history, ai) {
        ...
        var body = { model: cfg.model || 'mixtral-8x7b-32768', messages: messages, temperature: 0.6, max_tokens: 500, stream: false };
            ...
            function finishLocal(query, forceWeb) {
                ...
                wikiFetch(ai.title).then(function(w) {
                    if (w) {
                        ...
                        var txt2 = '🌐 В открытых источниках по запросу «' + ai.title + '» ничего подходящего не нашлось. ' + (local.text || ai.reason);
            ...
            aiBtn.on('hover:enter click keydown', function(e) {
                ...
                if (msgs.find('.sw-ai-message').length === 0) {
                    ...
                    var greet = apiCfg.url
                        ? 'Привет! Я подключён к ИИ и изучил «' + ai.title + '». Спрашивай что угодно!'
                        : 'Привет! Я изучил «' + ai.title + '». ИИ-модель не подключена — отвечаю по локальным данным и Википедии. Выбери вопрос кнопкой или напиши свой.';
    ...
    function registerController() {
        try {
            Lampa.Controller.add('should_watch_modal_enhanced', {
                ...
                left: function() { if (window._sw_blocknav) scrollStep(-1); },
                right: function() { if (window._sw_blocknav) scrollStep(1); },
                back: function() {
    ...
    function buildReadyInner(a) {
        ...
        if (cfg.ai_enabled) {
            ...
            aiChat = '<div class="sw-ai-chat" id="sw-ai-chat">' +
                ...
                '</div>' +
                '<div class="sw-ai-hint">Нажмите «Спросить ИИ» или выберите вопрос кнопкой</div>' +
                '</div>';
        }
        return '' +
            ...
            '<div class="sw-decision">' +
                '<div class="sw-decision-hint">Вердикт выше — брось кости или спроси ИИ</div>' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch Enhanced] v10.0 (Fixed scroll with all arrows, Groq AI support)');
    }
