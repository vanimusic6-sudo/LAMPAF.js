(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML = 
                /* Основные стили модального окна - боковая панель */
                '.sw-modal-wrapper{position:fixed;top:0;right:0;bottom:0;width:420px;max-width:90vw;background:rgba(0,0,0,0);z-index:10000;display:flex;flex-direction:column}' +
                '.sw-modal-content{padding:12px 14px 18px;color:#fff;font-family:' + DISPLAY + ';max-height:100vh;overflow-y:auto;-webkit-overflow-scrolling:touch;scroll-behavior:smooth;background:linear-gradient(180deg,rgba(12,12,12,.98) 0%,rgba(22,22,22,.96) 100%);display:flex;flex-direction:column;box-shadow:-4px 0 20px rgba(0,0,0,.5);border-left:1px solid rgba(255,255,255,.05)}' +
                '#sw-body{flex:1;display:flex;flex-direction:column;min-height:0;max-height:100vh}' +
                ...
                /* Загрузчик */
                '.sw-loader{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:48px 12px;color:#cbd5e1;flex:1}' +
                ...
                '.sw-modal-content{padding:12px 14px 20px;box-shadow:none;border-left:none}' +
                '.sw-modal-wrapper{width:100%;max-width:100vw;right:0;left:0;border-radius:0;background:rgba(0,0,0,.88)}' +
                '}' +
