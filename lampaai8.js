(function () {
    ...
    /* ===== НАВИГАЦИЯ TV (через Lampa.Controller.scroll) ===== */
    function initNavPoints(html) {
        try {
            if (!html || !html.length) return;
            window._sw_navPoints = [];
            ['.sw-dossier', '.sw-info-grid', '.sw-columns', '.sw-quote', '.sw-target-audience', '.sw-decision', '.sw-ai-chat.visible'].forEach(function(sel){
                var el = html.find(sel);
                if (el.length) { el.addClass('sw-nav-point'); window._sw_navPoints.push(el); }
            });
        ...
        try {
            ...
            aiBtn.on('hover:enter click keydown', function(e) {
                ...
                if (msgs.find('.sw-ai-message').length === 0) {
                    addAIMessage(msgs, 'bot', 'Привет! Я изучил «' + ai.title + '». Выбери вопрос кнопкой, нажми «🌐 Найти в сети» или напиши свой ниже.');
                }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch Enhanced] v8.0 (system keyboard, proper scroll, focus only on actions)');
    }
