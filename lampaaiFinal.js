(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-info-section{background:linear-gradient(90deg,rgba(96,165,250,.08),transparent);border:1px solid rgba(96,165,250,.18);padding:18px 22px;border-radius:14px;line-height:1.6;margin-bottom:24px;transition:background .25s ease,border-color .25s ease}' +
                '.sw-info-section:hover{background:linear-gradient(90deg,rgba(96,165,250,.14),transparent);border-color:rgba(96,165,250,.32)}' +
                '.sw-info-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;margin-top:14px}' +
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr){
            ...
            /* ====== ФЛАГИ ВЗРОСЛОГО КОНТЕНТА (расширенные) ====== */
            ...
            /* ====== ОПРЕДЕЛЕНИЕ СЕМЕЙНОСТИ (улучшенное) ====== */
            ...
            if (meta.production_companies && meta.production_companies.length > 0) {
                var famousStudios = ['warner bros','disney','universal','paramount','20th century','sony pictures','marvel','dc','pixar','dreamworks','hbo','netflix','a24','ghibli','laika'];
            ...
            return { pros: pros, cons: cons, review: rt, score: score, norm: norm, vClass: vClass, vWord: vWord, reason: reason, mode: dataRich ? 'TMDB' : 'TAGS', familyOK: familyOK, age: age, isAnim: isAnim, infoItems: infoItems };
        });
    }
    ...
    function buildReadyInner(a) {
        ...
        return '' +
            ...
            '</div>' +
            quote +
            '<div class="sw-info-section">' +
                '<div class="sw-title target">📊 Подробности</div>' +
                infoGrid +
            '</div>';
    }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch Enhanced] v11.0 (AI removed, improved content detection, new layout)');
    }
