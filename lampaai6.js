(function () {
    ...
    function saveSetting(k, v) { try { if (Lampa.Storage) Lampa.Storage.set(PLUGIN_ID + '_' + k, v); } catch(e) {} }
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-info-item.focus,.sw-info-item:hover{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.16);transform:translateY(-2px)}' +
                ...
                '.sw-col.focus{background:rgba(255,255,255,.06);border-color:rgba(255,255,255,.14)}' +
                ...
                '.sw-quote.focus{background:rgba(255,255,255,.06);border-left-color:rgba(133,194,94,.5)}' +
                ...
                '.sw-target-audience.focus{background:linear-gradient(90deg,rgba(133,194,94,.14),transparent);border-color:rgba(133,194,94,.32)}' +
                ...
                '.sw-ai-message{padding:11px 15px;border-radius:14px;margin-bottom:10px;font-size:.96em;line-height:1.55;max-width:88%;opacity:0;transform:translateY(8px);animation:swMsgIn .35s cubic-bezier(.25,.8,.25,1) forwards}' +
                ...
                '.sw-ai-input.focus,.sw-ai-input:focus{border-color:rgba(133,194,94,.55);background:rgba(255,255,255,.11)}' +
                ...
                '.sw-focusable{outline:none;cursor:pointer;scroll-margin-top:34px;scroll-margin-bottom:34px}' +
    ...
    /* ===== СКРОЛЛ: ищем реальный скролл-контейнер модалки ===== */
    ...
    function scrollContainerTo(el, center) {
        try {
            ...
            if (!c || !c.length) { refreshScrollContainer(); c = window._sw_scrollContainer; }
            if (!c || !c.length) return;
            var cn = c[0], en = el[0];
            var cRect = cn.getBoundingClientRect(), eRect = en.getBoundingClientRect();
            var delta = eRect.top - cRect.top;
            var target;
            if (center) {
                target = cn.scrollTop + delta - (cn.clientHeight / 2) + (eRect.height / 2);
            } else {
                if (eRect.top < cRect.top + 8) target = cn.scrollTop + delta - 20;
                else if (eRect.bottom > cRect.bottom - 8) target = cn.scrollTop + (eRect.bottom - cRect.bottom) + 20;
                else return;
            }
            var max = cn.scrollHeight - cn.clientHeight;
            if (target < 0) target = 0;
            if (target > max) target = max;
            if (max <= 0) target = 0;
            try { $(cn).stop(true, false).animate({ scrollTop: target }, 240); }
            catch(_) { cn.scrollTop = target; }
        } catch(e) { console.error('[SW] scrollContainerTo:', e); }
    }
    ...
    function focusFirstIn(point) {
        try {
            window._sw_navPoints.forEach(function(p){ p.find('.sw-focusable').removeClass('focus'); });
            var ft = point.find('.sw-focusable').first();
            if (!ft.length) ft = point;
            ft.addClass('focus');
            try { ft[0].focus({ preventScroll: true }); } catch(_) {}
            scrollContainerTo(ft, true);
    ...
    function swSetFocus(i) {
        try {
            ...
            try { el[0].focus({ preventScroll: true }); } catch(_) {}
            scrollContainerTo(el, false);
            try { Lampa.Controller.collectionFocus(el); } catch(_) {}
    ...
    function loadMeta(movie) {
        ...
        if (!id) return Promise.resolve({ kw: [], age: null, reviews: [], hasTrailer: false, enOv: '', budget: null, revenue: null, runtime: null, status: null, production_companies: [], production_countries: [], release_date: null });
        ...
        return Promise.all([
            ...
            if (arr[2] && arr[2].results)
                reviews = arr[2].results.slice(0, 5).map(function(r){ return { author: r.author || 'Аноним', text: (r.content || '').replace(/<[^>]+>/g, '').trim() }; }).filter(function(r){ return r.text.length > 20; });
            ...
            var budget = null, revenue = null, runtime = null, status = null, production_companies = [], production_countries = [], release_date = null;
            if (arr[5]) {
                budget = arr[5].budget || null; revenue = arr[5].revenue || null; runtime = arr[5].runtime || null; status = arr[5].status || null;
            ...
            var r = { kw: kw, age: age, reviews: reviews, hasTrailer: hasTrailer, enOv: enOv, budget: budget, revenue: revenue, runtime: runtime, status: status, production_companies: production_companies, production_countries: production_countries, release_date: release_date };
            _metaCache[id] = r;
            return r;
        });
    }
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr){
            ...
            if (meta.production_companies && meta.production_companies.length > 0) {
                ...
                var hasFamousStudio = meta.production_companies.some(function(c){ return famousStudios.some(function(s){ return c.toLowerCase().indexOf(s) >= 0; }); });
            ...
            var ai = {
                ...
                cast: cast.slice(0, 6),
                directors: dirs,
                writers: wrts.slice(0, 3),
                rating: rating, votes: votes, runtime: runtime, year: yr, age: age,
                budget: meta.budget, revenue: meta.revenue,
                studios: meta.production_companies.slice(0, 3),
                countries: meta.production_countries.slice(0, 3),
                hasTrailer: meta.hasTrailer,
                familyOK: familyOK, isAnim: isAnim,
                reviewTone: rt.tone,
                ...
                vWord: vWord, vClass: vClass, reason: reason,
                pros: pros, cons: cons
            };
    ...
    function addAIMessage(container, type, text) {
        ...
        try { container[0].scrollTop = container[0].scrollHeight; } catch(e) {}
    }
    function aiThinking(container) {
        var t = $('<div class="sw-ai-message bot"><div class="sw-ai-typing"><span></span><span></span><span></span></div></div>');
        container.append(t);
        try { container[0].scrollTop = container[0].scrollHeight; } catch(e) {}
        return t;
    }

    function generateSmartAIResponse(query, ai) {
        ...
        if (/^(привет|здравствуй|хай|хеллоу|hi|hello|йо)\b/.test(t) || t.indexOf('привет') === 0) {
            return 'Привет! Я изучил карточку «' + title + '». Спрашивай что угодно: о сюжете, актёрах, рейтинге, цензе — или стоит ли тратить вечер.';
        }
        if (/спасиб|благодар|thanks/.test(t)) return 'Пожалуйста 🙂 Если что — я тут, задавай ещё.';

        // Сюжет / о чём
        if (/сюжет|о чём|про что|описание|суть|истори|пересказ|что за фильм|что за сериал/.test(t)) {
            if (ai.overview) return 'Если коротко: ' + ai.overview + (ai.genres.length ? ' По жанру это ' + pick(ai.genres, 4).toLowerCase() + '.' : '');
            return 'Краткого описания в базе нет, но по тегам это ' + (ai.genres.length ? pick(ai.genres, 4).toLowerCase() : kind) + '. Полный синопсис смотри в блоке «Подробно» на карточке.';
        }

        // Жанр
        if (/жанр|категор|направлен/.test(t)) {
            return ai.genres.length ? 'Жанры: ' + pick(ai.genres, 6) + '.' : 'Жанры в базе не указаны.';
        }

        // Актёры / кто играет
        if (/актер|актёр|играет|играют|в ролях|каст|звезд|звёзд|снималс|снимались/.test(t)) {
            return ai.cast.length ? 'В главных ролях: ' + pick(ai.cast, 6) + '.' : 'Список актёров не подгрузился из базы.';
        }
        ...
        if (/режиссер|режиссёр|постановщик|снял кто|автор|сценарист|сценарий/.test(t)) {
            ...
            return (d || w) ? (d + w) : 'Данные о съёмочной группе не подгрузились.';
        }

        // Рейтинг / оценка / отзывы
        ...
        // Стоит ли смотреть
        if (/стоит ли|рекоменд|посовету|смотреть или нет|заценить|тратить время/.test(t)) {
            return 'Мой вердикт: ' + ai.vWord + '. ' + ai.reason;
        }

        // Длительность
        if (/длительн|сколько длится|минут|хронометраж|продолжительн|серий|сезонов/.test(t)) {
            return ai.runtime ? 'Хронометраж: ' + ai.runtime + ' мин.' + (ai.type === 'tv' ? ' Это сериал, так что серий там побольше.' : '') : 'Точный хронометраж в базе не указан.';
        }

        // Год
        if (/год|когда вышел|дата выход|выпущен|свеж|новинк/.test(t)) {
            return ai.year ? ('Вышел в ' + ai.year + ' году.' + (ai.year >= new Date().getFullYear() - 1 ? ' То есть довольно свежее.' : '')) : 'Год выхода не указан.';
        }
        ...
        if (/бюджет|стоил|сборы|касса|заработал|окупил|кассов/.test(t)) {
            ...
            var ok = (ai.budget && ai.revenue) ? (ai.revenue > ai.budget * 2 ? ' В прокате сработал отлично.' : (ai.revenue > ai.budget ? ' В прокате окупился.' : ' В прокате, увы, не окупился.')) : '';
            return (b || rv) ? (b + rv + ok) : 'Данных о бюджете и сборах в базе нет (часто так бывает у сериалов).';
        }

        // Страна / студия
        if (/стран|студия|производств|компани|кто снял студи/.test(t)) {
            var c = ai.countries.length ? 'Страна: ' + pick(ai.countries, 3) + '.' : '';
            var st = ai.studios.length ? ' Студия/производство: ' + pick(ai.studios, 3) + '.' : '';
            return (c || st) ? (c + st) : 'Данные о производстве не подгрузились.';
        }

        // Детям / семейный
        if (/детск|для детей|семейн|ребенк|ребёнк|можно ли с детьм/.test(t)) {
            if (ai.familyOK) return 'Да, подходит для семейного просмотра — можно смотреть с детьми.' + (ai.age !== null ? ' Возрастная метка: ' + ai.age + '+.' : '');
            var why = [];
            if (ai.flags.viol) why.push('насилие');
            if (ai.flags.profanity) why.push('мат');
            if (ai.flags.nudity || ai.flags.sex) why.push('откровенные сцены');
            if (ai.flags.drugs) why.push('тема наркотиков');
            if (ai.age !== null && ai.age >= 16) why.push('ценз ' + ai.age + '+');
            return 'С детьми — осторожно: это не семейный контент.' + (why.length ? ' Причина: ' + why.join(', ') + '.' : '') + (ai.age !== null ? ' Метка ' + ai.age + '+.' : '');
        }

        // Ценз / возраст
        if (/ценз|возраст|\d\+|рейтинг возр/.test(t)) {
            return ai.age !== null ? 'Возрастной ценз: ' + ai.age + '+.' : 'Точный ценз в базе не найден — ориентируйся на предупреждения в блоке «против».';
        }

        // Контент-предупреждения
        if (/насили|кров|жесток|убийств/.test(t)) return ai.flags.viol ? 'Да, есть жестокие/кровавые сцены.' : 'Сцен насилия по данным базы не обнаружено.';
        if (/мат|нецензур|ругательств|брань/.test(t)) return ai.flags.profanity ? 'Да, нецензурная лексика присутствует.' : 'Мата по данным базы нет.';
        if (/нагот|обнаж|эротик|секс|постельн/.test(t)) return (ai.flags.nudity || ai.flags.sex) ? 'Да, есть откровенные/сексуальные сцены.' : 'Откровенных сцен по данным базы нет.';
        if (/наркот|выпивк|алкогол|курени|сигарет/.test(t)) {
            var w2 = [];
            if (ai.flags.drugs) w2.push('наркотики');
            if (ai.flags.alcohol) w2.push('алкоголь');
            if (ai.flags.smoke) w2.push('курение');
            return w2.length ? ('Да, затрагивается: ' + w2.join(', ') + '.') : 'По данным базы — нет.';
        }

        // Трейлер
        if (/трейлер|ролик|посмотреть превью/.test(t)) return ai.hasTrailer ? 'Да, трейлер есть — можно глянуть на карточке и составить впечатление за пару минут.' : 'Трейлера в базе не нашлось.';

        // Настроение / атмосфера / вайб
        if (/настроен|атмосфер|вайб|напряж|весел|грустн|мрачн|легк/.test(t)) {
            ...
            return v.length ? ('По настроению — скорее ' + v.join(', ') + '.') : 'Настроение лучше считать по трейлеру и отзывам.';
        }

        // Плюсы/минусы кратко
        if (/плюс|минус|за и против|аргумент/.test(t)) {
            return 'Главное «за»: ' + (ai.pros[0] || '—') + '. Главное «против»: ' + (ai.cons[0] || '—') + '.';
        }

        // Fallback — осмысленный, не «я не знаю»
        var fb = 'По «' + title + '» (' + kind + ') у меня есть такие факты: ';
        var facts = [];
        ...
        fb += ' Вердикт — ' + ai.vWord + '.';
        if (ai.overview) fb += ' По сути: ' + (ai.overview.length > 140 ? ai.overview.substring(0, 140) + '…' : ai.overview);
        fb += ' Если нужна деталь, которой нет в карточке (например, факты со съёмок или спойлеры) — её стоит глянуть в описании/трейлере или поиском: я работаю только по данным TMDB и не выхожу в интернет.';
        return fb;
    }
        ...
        try {
            ...
            function ask(query) {
                ...
                setTimeout(function(){
                    ...
                    addAIMessage(msgs, 'bot', generateSmartAIResponse(query, ai));
                }, 650 + Math.random() * 350);
            }
            ...
            aiBtn.on('hover:enter click keydown', function(e){
                ...
                if (window._sw_aiChatOpen) {
                    if (msgs.find('.sw-ai-message').length === 0) {
                        addAIMessage(msgs, 'bot', 'Привет! Я изучил «' + ai.title + '». Выбери вопрос кнопкой или напиши свой в поле ниже.');
                    }
                    initNavPoints(html);
                    setTimeout(function(){ var c = chat.find('.sw-ai-chip').first(); if (c.length) { c.addClass('focus'); try { Lampa.Controller.collectionFocus(c); } catch(_){} scrollContainerTo(c, false); } }, 80);
                } else {
            ...
            input.on('keydown', function(e){
                if (e.keyCode === 13 && !e.shiftKey) { e.preventDefault(); sendBtn.trigger('click'); }
            });
        ...
        try {
            Lampa.Controller.add('should_watch_modal_enhanced', {
                ...
                up: function() {
                    ...
                    if (window._sw_navPoints && window._sw_navPoints.length > 1) scrollToNavPoint(window._sw_currentNavPoint - 1);
                    else swSetFocus((window._sw_focusIdx || 0) - 1);
                },
                down: function() {
                    if (!window._sw_blocknav) return;
                    if (window._sw_navPoints && window._sw_navPoints.length > 1) scrollToNavPoint(window._sw_currentNavPoint + 1);
                ...
                back: function() {
                    if (window._sw_aiChatOpen) {
                        ...
                        chat.removeClass('visible');
                        initNavPoints(window._sw_currentModalHtml);
    ...
    function buildReadyInner(a) {
        ...
        if (a.infoItems && a.infoItems.length) {
            infoGrid = '<div class="sw-info-grid">' + a.infoItems.map(function(it){
                return '<div class="sw-info-item sw-focusable" tabindex="0"><div class="sw-info-label">' + esc(it.label) + '</div><div class="sw-info-value">' + esc(it.value) + '</div></div>';
            }).join('') + '</div>';
        }
        ...
        if (cfg.ai_enabled) {
            ...
            aiChat = '<div class="sw-ai-chat" id="sw-ai-chat">' +
                ...
                '<div class="sw-ai-suggestions">' +
                    ...
                    '<div class="sw-ai-chip sw-focusable" data-q="Какое настроение у фильма?" tabindex="0">Настроение</div>' +
                '</div>' +
                '<div class="sw-ai-input-row">' +
                    '<textarea class="sw-ai-input sw-focusable" id="sw-ai-input" placeholder="Или напиши свой вопрос…" rows="1" tabindex="0"></textarea>' +
                    '<button class="sw-ai-send sw-focusable" id="sw-ai-send" tabindex="0">➤</button>' +
                '</div>' +
                '<div class="sw-ai-hint">На ТВ удобнее кнопки‑вопросы выше · на ПК/телефоне можно печатать в поле</div>' +
                '</div>';
        }
        ...
        return '' +
            ...
            '<div class="sw-columns">' +
                '<div class="sw-col sw-focusable" tabindex="0"><div class="sw-title pros">✓ Аргументы за</div><ul class="sw-list">' + a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
                '<div class="sw-col sw-focusable" tabindex="0"><div class="sw-title cons">✗ Аргументы против</div><ul class="sw-list">' + a.cons.map(function(c){ return '<li>' + esc(c) + '</li>'; }).join('') + '</ul></div>' +
            '</div>' +
            quote +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch Enhanced] v5.0 (full file, real scroll-parent, conversational AI)');
    }
