(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML = 
                ...
                '.sw-modal-wrapper{position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:10000;display:flex;flex-direction:column;overflow:hidden}' +
                '.sw-modal-content{padding:16px 18px 24px;color:#fff;font-family:' + DISPLAY + ';min-height:100vh;max-height:100vh;overflow-y:auto;-webkit-overflow-scrolling:touch;scroll-behavior:smooth;background:linear-gradient(180deg,rgba(15,15,15,.98) 0%,rgba(25,25,25,.95) 100%)}' +
                ...
                /* Загрузчик */
                '.sw-loader{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:48px 16px;color:#cbd5e1;min-height:60vh}' +
                '.sw-loader-dice{font-size:2.8em;line-height:1;animation:swBounce 1s ease-in-out infinite;filter:drop-shadow(0 4px 12px rgba(133,194,94,.35))}' +
                '.sw-loader-text{font-size:1.05em;font-weight:600;min-height:1.4em;transition:opacity .25s;color:#94a3b8;text-align:center}' +
                '.sw-loader-bar{width:180px;height:2px;border-radius:2px;background:rgba(255,255,255,.08);overflow:hidden;position:relative}' +
                ...
                '.sw-modal-content{padding:12px 14px 20px}' +
                '}' +
    ...
    function showModal(movie) {
        try {
            ...
            var html = $('<div class="sw-modal-wrapper"><div class="sw-modal-content"><div id="sw-body"><div class="sw-loader"><div class="sw-loader-dice">🎲</div><div class="sw-loader-text" id="sw-loader-text">' + phrases[0] + '</div><div class="sw-loader-bar"></div></div></div></div>');
